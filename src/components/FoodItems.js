...
const FoodItems = ({ name, price, imageId, description, quantity }) => {
    ...
    return (
        <div className="food-item p-5 m-8 shadow-lg bg-pink-50 w-1/2 mx-auto flex justify-between items-center">
            <div className="flex items-center w-[50%]">
                <img src={IMG_CDN_URL + imageId} className="w-12" alt={name} />
                <h2 className="font-medium ml-3">{name}</h2>
            </div>
            ...
            <h3>₹{(price / 100 * quantity).toFixed(2)}</h3>
